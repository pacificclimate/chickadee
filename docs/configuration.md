...
```
poetry run chickadee start --help
...
```
poetry run chickadee start --hostname localhost --port 5000
```
...
For example change the hostname (_demo.org_) and logging level:
...
poetry run chickadee start -c etc/custom.cfg
```
