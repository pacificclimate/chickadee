...
from chickadee.utils import get_package, collect_args, set_general_options, set_ca_options, set_qdm_options
...
def test_set_general_options(
    ...
    assert list(base.getOption("target.units")) == [tasmax_units, tasmin_units, pr_units]
    assert base.getOption("max.GB")[0] == max_gb

@pytest.mark.parametrize(
    ("num_analogues", "delta_days", "start_date", "end_date", "trimmed_mean", "tol", "expon"),
    [
        (
            25,
            40.0,
...
def test_set_qdm_options(
    ...
    assert [int(tau) for [tau] in list(base.getOption("tau"))] == [pr_tau, tasmax_tau, tasmin_tau]
    assert [bool(season[0]) for season in base.getOption("seasonal")] == [pr_seasonal, tasmax_seasonal, tasmin_seasonal]
    assert [bool(num[0]) for num in base.getOption("ratio")] == [pr_ratio, tasmax_ratio, tasmin_ratio]
