...
def handle_run_chickadee(arg):
    ...
    if dataset.value == "PNWNAmet":
        chickadee_params["out_file"] = f"{gcm_var}_{dataset.value}_target_{period.value}_on-demand.nc"
    else:
        if not canesm5_run.disabled:
            chickadee_params["out_file"] = f"{gcm_var}_{dataset.value}_{technique.value}_{model.value}_{canesm5_run.value}_{scenario.value}_target_{period.value}_on-demand.nc"
        else:
            chickadee_params["out_file"] = f"{gcm_var}_{dataset.value}_{technique.value}_{model.value}_{scenario.value}_target_{period.value}_on-demand.nc"
